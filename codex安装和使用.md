...
- **添加令牌：** 操作步骤为左上方点击 `控制台` → 左侧菜单选择 `令牌管理` → 点击 `添加令牌`。根据需要设置令牌名称、过期时间和额度，令牌分组会影响能使用的模型，需要在 `模型广场` 中查看需要的模型属于哪个分类
...
- **配置应用:** 
  ...
  2. 添加配置：为了使用New API中提供的模型需要在config.toml中添加下面配置，并保存后重启
...
![image-20260608093838618](assets/image-20260608093838618.png)

- **检查并发布：** 生成完成后可以直接点击 `审核` 进行检查。检查完毕后需要到IDEA使用 Bap插件提交并发布

![image-20260608094041983](assets/image-20260608094041983.png)
...
3.  必填内容中，  **“函数编号”** 为方便管理通常采用 **工作室ID与函数名** 用下划线隔开的方式命名，  **“操作名称”** 为该操作在界面中显示的名称。
...
2.如果希望通过表单组件交互触发可在【视图调整】中组件的【运行时】中添加，例如在一个请假表单中输入开始和结束时间，要算出准确的请假时长就可以使用操作函数进行计算，避免了在赋值指令中写过长的代码。
...
2.4   **“事件参数”** 可以配置操作函数所需的上下文参数，参数名和操作函数@MethodDeclare中 **“exampleValue”** 填写的**$**符包裹的文字一致。需要注意这里填的不能是octo.cm.enums.GpfContextSystemVarKey和octo.cm.enums.ContextSystemVarKey类中包含的系统固定的参数名称。
...
2.6填写配置指令并保存后即可在【预览应用】中测试预览![image-20260601104908014](assets/image-20260601104908014.png)![image-20260601104922696](E:/文档/kwaidoo/assets/image-20260601104922696.png)
