...
import cmn.http.servlet.impl.OutputStreamActionResult;
import cmn.http.servlet.impl.OutputStreamRangeActionResult;
import cmn.http.servlet.impl.SseConnection;
import cmn.http.servlet.impl.SseConnectionManager;
...
public class AppHttpResponseHandler implements HttpResponseHandler {

	/**
	 *
	 */
	private static final long serialVersionUID = 4385823295965345513L;
	public final static String LOG = AppHttpResponseHandler.class.getSimpleName();
	@Override
...
//					return actionResult;
	...
	public static void handleFluxStream(HttpServletRequest request, HttpServletResponse response, Flux<?> flux)
		...
		// 将心跳流与实际流合并
		Flux<String> heartbeat = Flux.interval(Duration.ofSeconds(15)).map(i -> ":\n\n"); // SSE 心跳
		Flux<String> dataFlux = flux.map(AppHttpResponseHandler::formatSSE);
...
//		subRef.set(sub);
	...
	public static String formatSSE(Object data) {
		if (data instanceof String) {
			...
			String errorMsg = rootCause != null ? rootCause.getMessage() : data.toString();
			return "event: error\ndata: " + errorMsg + "\n\n";
		} else {
			return "data: " + JsonUtil.toJson(data) + "\n\n";
		}
