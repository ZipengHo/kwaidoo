...
public class SpeedTestServlet extends HttpServlet {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -6035685759339749980L;
	/**
...
//		String servletPath = request.getPathInfo();
		String servletPath = request.getParameter("size");
		if (servletPath == null || servletPath.isEmpty() || "/".equals(servletPath)) {
			...
			try (PrintWriter output = response.getWriter()) {
				...
				output.println("=====");
				output.println();
				output.print("Test download:	curl -o /dev/null ");output.print(request.getRequestURL());output.println("?size=<size>[kKmMgG]");
...
//		servletPath = servletPath.substring(1); // remove leading "/"
		...
		System.out.println("测试下行速度：" + servletPath);
